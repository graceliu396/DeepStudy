...
def update_checkpoint_container(data):
    try:
        checkpoint_container.upsert_item(data)
...
def fetch_checkpoint_by_user_and_file_id(userId, fileId):
    try:
        query = f"SELECT * FROM c WHERE c.userId = '{userId}' AND c.fileId = '{fileId}'"
        items = list(checkpoint_container.query_items(query=query, enable_cross_partition_query=True))
        print(
            f"[DEBUG] Fetched {len(items)} checkpoint data for userId: {userId}, fileId: {fileId}")
        return items
    except Exception as e:
        print(
            f"[ERROR] Error fetching checkpoint data for userId: {userId}, fileId: {fileId}: {e}")
        raise e
    
def fetch_chat_history_by_user_file_session_id(userId, fileId, sessionId):
    try:
        query = f"SELECT * FROM c WHERE c.userId = '{userId}' AND c.fileId = '{fileId}' AND c.sessionId = '{sessionId}'"
...
def delete_upload_file(userId, fileId):
    try:
        query = f"SELECT * FROM c WHERE c.userId = '{userId}' AND c.fileId = '{fileId}'"
        items = list(file_container.query_items(query=query, enable_cross_partition_query=True))
        if len(items) == 0:
            print(f"[DEBUG] No file data found for userId: {userId}, fileId: {fileId}")
            return
        for item in items:
            file_container.delete_item(item, partition_key=[userId, fileId])
            print(f"[DEBUG] Deleted file data for userId: {userId}, fileId: {fileId}")
    except Exception as e:
        print(
            f"[ERROR] Error deleting file data for userId: {userId}, fileId: {fileId}: {e}")
        raise e
    
def delete_checkpoint(userId, fileId):
    try:
        query = f"SELECT * FROM c WHERE c.userId = '{userId}' AND c.fileId = '{fileId}'"
        items = list(checkpoint_container.query_items(query=query, enable_cross_partition_query=True))
        if len(items) == 0:
            print(f"[DEBUG] No checkpoint data found for userId: {userId}, fileId: {fileId}")
            return  
        for item in items:
            checkpoint_container.delete_item(item, partition_key=[userId, fileId])
            print(f"[DEBUG] Deleted checkpoint data for userId: {userId}, fileId: {fileId}")
    except Exception as e:
        print(f"[ERROR] Error deleting checkpoint data for userId: {userId}, fileId: {fileId}: {e}")
        raise e
...
def create_checkpoint_record(checkpoint_data):
    try:
        checkpoint_container.upsert_item(checkpoint_data)
        print(f"[DEBUG] Checkpoint record created: {checkpoint_data}")
    except Exception as e:
        print(f"[ERROR] Error creating checkpoint record: {e}")
        raise e
